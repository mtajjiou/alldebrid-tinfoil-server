    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if check.status_code == 503:
                logging.error("ALERT: VPS IS BLOCKED BY ALLDEBRID (503 Service Unavailable).")
                logging.error("Your VPS IP address is blacklisted. Download will likely fail.")
                logging.error("Check Alldebrid website for more info on Server IPs.")
                ...
                logging.warning(f"Warning: Alldebrid returned code {check.status_code} during connectivity check.")
                
    except Exception as e:
        logging.warning(f"Unable to verify Alldebrid link status: {e}")
