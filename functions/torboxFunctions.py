...
async def getDownloadLink(download_type: str, download_id: int, file_id: int = 0):
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            # Fix: Torbox API parameters might need correct mapping
            params = {"token": TORBOX_API_KEY, id_type: download_id, "file_id": file_id} 
            # Note: The original code had {id_type: download_id, file_id: file_id} which implies file_id var is key? 
            # Looking at original code: `params = {"token": TORBOX_API_KEY, id_type: download_id, file_id: file_id}`
            # If default file_id param name is "file_id", then it's fine.
            # I will trust the original code structure but use string keys for safety if possible?
            # Original: params = {"token": TORBOX_API_KEY, id_type: download_id, file_id: file_id}
            # This uses the value of argument file_id as the key? No, in py param=val.
            # Ah, the original code likely meant "file_id": file_id.
            # I'll stick to strict dict:
            params = {"token": TORBOX_API_KEY, id_type: download_id, "file_id": file_id}
