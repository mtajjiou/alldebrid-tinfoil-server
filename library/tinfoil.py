...
def errorMessage(message: str, error_code: str):
    ...
    - error_code: a string which is a documented error code for a developer to know. For error code documentation, view Alldebrid API docs.

    Returns:
    - error: a dict with the 'error' key which is read by Tinfoil. Documentation here: https://blawar.github.io/tinfoil/custom_index/
    """

    return {"error": f"Alldebrid\n\n{message}\n\nError: {error_code}"}
